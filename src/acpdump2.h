...
#include "ip2.h"
        ...
        #include <windef.h>
        #include <winbase.h>
        #define ACPDUMP_LOCK_VAR    static LONG acpdump_is_writing = 0;
        #define ACPDUMP_LOCK_INIT
        #define ACPDUMP_LOCK_START  while(InterlockedExchange(&acpdump_is_writing, 1)) Sleep(0);
        #define ACPDUMP_LOCK_END    InterlockedExchange(&acpdump_is_writing, 0);
    #else
        #include <pthread.h>
        #include <time.h>
        #define ACPDUMP_LOCK_VAR    static pthread_mutex_t  *acpdump_is_writing = NULL;
        #define ACPDUMP_LOCK_INIT   if(!acpdump_is_writing) { \
                                        acpdump_is_writing = calloc(1, sizeof(pthread_mutex_t)); \
                                        if(!acpdump_is_writing) return -1; \
                                        pthread_mutex_init(acpdump_is_writing, NULL); \
                                    }
        #define ACPDUMP_LOCK_START  while(pthread_mutex_trylock(acpdump_is_writing)) usleep(0);
        #define ACPDUMP_LOCK_END    pthread_mutex_unlock(acpdump_is_writing);
    #endif

        /* no windows and no pthread
        static int acpdump_is_writing = 0;
        #define ACPDUMP_LOCK_START  while(acpdump_is_writing) { sleep(0); } acpdump_is_writing = 1;
        #define ACPDUMP_LOCK_END    acpdump_is_writing = 0;
        */
#else
    #define ACPDUMP_LOCK_INIT
...
int putxx(FILE *fd, uint32_t num, int bits) {
    ...
    bytes = bits >> 3;
    for(i = 0; i < bytes; i++) {
        if(fputc(num >> (i << 3), fd) < 0) return -1;
    }
    return 0;
}
...
int create_acp(FILE *fd) {
    if(!fd) return -1;
    if(putxx(fd, 0xa1b2c3d4, 32) < 0) return -1;
    if(putxx(fd, 2,          16) < 0) return -1;
    if(putxx(fd, 4,          16) < 0) return -1;
    if(putxx(fd, 0,          32) < 0) return -1;
    if(putxx(fd, 0,          32) < 0) return -1;
    if(putxx(fd, 65535,      32) < 0) return -1;
    if(putxx(fd, 1,          32) < 0) return -1;
    fflush(fd);
    return 0;
}
...
int acp_dump(FILE *fd, int type, int protocol, uint32_t src_ip, uint16_t src_port, uint32_t dst_ip, uint16_t dst_port, uint8_t *data, int len, uint32_t *seq1, uint32_t *ack1, uint32_t *seq2, uint32_t *ack2, uint32_t seed) {
    ...
    if((sizeof(acp_pck) + sizeof(ethdata) + size) > 0xffff) { // divides the packet if it's too big
        ...
        while(size > 0) {
            ...
            acp_dump(fd, type, protocol, src_ip, src_port, dst_ip, dst_port, data, len, seq1, ack1, seq2, ack2, seed);
            size -= len;
            data += len;
        }
        return 0;
    ...
    if(!tp) {
        ...
        tcp.ack_seq      = net32(((tcp.flags == TH_SYN) ? 0 : seed) + *ack1);
...
int acp_dump_handshake(FILE *fd, int type, int protocol, uint32_t src_ip, uint16_t src_port, uint32_t dst_ip, uint16_t dst_port, uint8_t *data, int len, uint32_t *seq1, uint32_t *ack1, uint32_t *seq2, uint32_t *ack2, uint32_t seed) {
    ...
    *ack2 = 0;
    if(!((!type && !protocol) || ((type == 1) && !protocol) || (protocol == 6))) {
        return 0; // don't make handshake if it's not tcp
    }

    *seq1 = 1;
    *ack1 = 0;
    if(acp_dump(fd, type, protocol, src_ip, src_port, dst_ip, dst_port, NULL, 0, seq1, ack1, seq2, ack2, seed) < 0) return -1;

    *ack2 = *seq1 + 1;
    *seq2 = 1;
    if(acp_dump(fd, type, protocol, dst_ip, dst_port, src_ip, src_port, NULL, 0, seq2, ack2, seq1, ack1, seed) < 0) return -1;

    *ack1 = *seq2 + 1;
    (*seq1)++;
    if(acp_dump(fd, type, protocol, src_ip, src_port, dst_ip, dst_port, data, len, seq1, ack1, seq2, ack2, seed) < 0) return -1;
